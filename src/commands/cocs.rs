        ...
        Some(player_tag) => {
            ...
            let player = match coc_client.get_player(player_tag).await {
                ...
                Err(why) => {
                    ...
                    msg.channel_id.say(&ctx.http, "Error getting player").await?;
                    return Ok(());
                }
            };
            msg.channel_id.say(&ctx.http, format!("Player: {}", player.name)).await?;
        }
        None => {
            msg.channel_id.say(&ctx.http, "No player tag provided").await?;
        }
    }
    Ok(())
}
